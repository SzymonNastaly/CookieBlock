    ...
    for (let path of cookiePaths) {
      getExtensionFile(path, "json", (result) => {
        if (
          ...
          result !== null &&
          ("name_match" in result ||
            "domain_match" in result ||
            "domain_regex" in result)
        ) {
...
const maybeRestoreCBLKVar = async function (cValue, varName) {
  if (cValue === undefined) {
    ...
    switch (varName) {
      case "cblk_userpolicy":
        cblk_userpolicy = await getStorageValue(
          chrome.storage.sync,
          "cblk_userpolicy",
        );
        break;
      ...
      case "cblk_pause":
        ...
        break;
      case "cblk_ulimit":
        cblk_ulimit = await getStorageValue(
          chrome.storage.local,
          "cblk_ulimit",
        );
        break;
      case "cblk_hconsent":
        cblk_hconsent = await getStorageValue(
          chrome.storage.sync,
          "cblk_hconsent",
        );
        break;
      case "cblk_exglobal":
        cblk_exglobal = await getStorageValue(
          chrome.storage.sync,
          "cblk_exglobal",
        );
        break;
      ...
      case "cblk_exanal":
        ...
        break;
      case "cblk_exadvert":
        cblk_exadvert = await getStorageValue(
          chrome.storage.sync,
          "cblk_exadvert",
        );
        break;
      case "cblk_mintime":
        cblk_mintime = await getStorageValue(
          chrome.storage.sync,
          "cblk_mintime",
        );
        break;
      case "cblk_knowncookies": {
        cblk_knowncookies = await getStorageValue(
          chrome.storage.sync,
          "cblk_knowncookies",
        );
      ...
      case "cblk_useinternal":
        cblk_useinternal = await getStorageValue(
          chrome.storage.sync,
          "cblk_useinternal",
        );
        break;
...
openDBRequest.onerror = function (event) {
  console.error(
    `Failed to open history database with error code: ${event.target.errorCode}`,
  );
};
...
const insertCookieIntoStorage = function (serializedCookie) {
  if (historyDB !== undefined) {
    ...
    let putRequest = historyDB
      ...
      .objectStore("cookies")
      .put(serializedCookie, ckey);
    putRequest.onerror = function (event) {
      console.error(
        `Failed to insert cookie (${ckey}) into IndexedDB storage: ${event.target.errorCode}`,
      );
    };
  } else {
    console.error(
  ...
  if (historyDB !== undefined) {
    ...
    return new Promise((resolve, reject) => {
      request.onerror = function (event) {
        ...
        reject(
          `Error on retrieving cookie (${ckey}) -- Error code ${event.target.errorCode}`,
        );
      };
    ...
    console.error(
      "Could not retrieve cookie because database connection is closed!",
    );
    return new Promise((resolve, reject) => {
...
const updateFEInput = async function (storedFEInput, rawCookie) {
  ...
  if (updateArray.length >= updateLimit) updateArray.shift();
  ...
  console.assert(
    updateArray.length <= updateLimit,
...
const cookieLookup = function (cookieDat, knownCookiesArg) {
  ...
  if (cookieDomain in knownCookiesArg["domain_match"]) {
    return nameLookup(
      cookieName,
      knownCookiesArg["domain_match"][cookieDomain],
    );
  } else {
...
const classifyCookie = async function (cookieDat, feature_input) {
  ...
  if (label === -1) {
    ...
    recordDebugTimings(performance.now() - startTime, 1);

    // Prediction timing
    startTime = performance.now();
    label = await predictClass(features, cblk_pscale);
    recordDebugTimings(performance.now() - startTime, 2);
  } else {
    debug_Nskipped++;
  }
...
const makePolicyDecision = async function (cookieDat, label) {
  ...
  try {
    ...
    console.error(
      `Failed to retrieve exception storage value. Error: ${err.message}`,
    );
  ...
  if (skipRejection) {
    console.debug(
      `Cookie found on allowlist for category '${cName}': '${cookieDat.name}';'${cookieDat.domain}';'${cookieDat.path}'`,
    );
          ...
          remResultHTTPS = chrome.cookies.remove(
            {
              ...
              url:
                "http://" +
                domainRemoveNoise(cookieDat.domain) +
                cookieDat.path,
              storeId: cookieDat.storeId,
            },
            (remResultHTTP) => {
              if (remResultHTTP === null) {
                ...
                console.error(
                  "Removal failed: Could not find cookie (%s;%s;%s) in storage. Assigned label: (%s)",
                  cookieDat.name,
                  cookieDat.domain,
                  cookieDat.path,
                  cName,
...
const handleCookie = async function (
  ...
  try {
    ...
    console.error(
      "Retrieving or updating cookie failed unexpectedly.\nOriginal error: " +
        err.message,
    );
  }
  ...
  recordDebugTimings(performance.now() - startTime, 0);
  console.assert(
    serializedCookie !== undefined,
    "Cookie object was still undefined!",
  );
  ...
  if (cblk_exglobal.includes(ckDomain)) {
    console.debug(
      `Cookie found in domain allowlist: (${constructKeyFromCookie(newCookie)})`,
    );
  } else {
    ...
    if (overrideTimeCheck || clabel === -1 || elapsed > cblk_mintime) {
      ...
      console.debug(
        "Perform Prediction: Cookie (%s;%s;%s) receives label (%s)",
        newCookie.name,
        newCookie.domain,
        newCookie.path,
      ...
      debug_Nskipped++;
      console.debug(
        "Skip Prediction: Cookie (%s;%s;%s) with label (%s)",
        newCookie.name,
        newCookie.domain,
        newCookie.path,
        classIndexToString(clabel),
    ...
    if (cblk_pause) {
      console.debug(
        `Pause Mode Removal Skip: Cookie Identifier: ${constructKeyFromCookie(newCookie)} -- Assigned Label: ${classIndexToString(clabel)}`,
      );
    } else {
...
const constructHistoryJSON = function (type) {
  console.assert(
    historyDB !== undefined,
    "Cookie database link was undefined!",
  );
  ...
  return new Promise((resolve, reject) => {
    cursor.onsuccess = function (event) {
      ...
      if (cursor) {
        if (
          target_label === null ||
          cursor.value.current_label === target_label
        ) {
    ...
    cursor.onerror = (event) => {
      reject(`Error Code: ${event.target.errorCode}`);
    };
  });
};
  ...
  if (request.classify_all) {
    chrome.cookies.getAll({}, async (allCookies) => {
      if (chrome.runtime.lastError) {
        console.error(
          "Encountered an error when trying to retrieve all cookies: " +
        ...
        sendResponse({
          response: "All cookies classified and policy enforced.",
        });
      }
    });
    return true;
    ...
    let updateCookieProcess = async () => {
      ...
      sendResponse({
        response: `Cookie label updated for: ${actualCookie.name};${actualCookie.domain};${actualCookie.path}`,
      });
    };
    updateCookieProcess();
    return true;
...
getExtensionFile(
  chrome.runtime.getURL("/ext_data/known_cookies.json"),
  "json",
  (result) => {
    for (let k of Object.keys(result["domain_regex"])) {
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    chrome.tabs.create({
      active: true,
      url: "/options/cookieblock_setup.html",
    });
  }
});
