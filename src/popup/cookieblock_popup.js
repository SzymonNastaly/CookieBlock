...
const ignoredPages =
  /^(view-source:|moz-extension:|about:|chrome-extension:|chrome:)/;
...
const popupSetup = async function () {
  ...
  pauseCheckbox.checked = await getStorageValue(
    chrome.storage.local,
    "cblk_pause",
  );
  popupLogo.src = pauseCheckbox.checked
    ? "/icons/gs-cookieblock-96.png"
    : "/icons/cookieblock-96.png";
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
    ...
    if (currentURL.match(ignoredPages)) {
      ...
      if (sanitizedDomain) {
        let exglobal = await getStorageValue(
          chrome.storage.sync,
          "cblk_exglobal",
        );
...
pauseCheckbox.addEventListener("click", async () => {
  ...
  chrome.action.setIcon(pauseStatus ? grayScaleIcon : defaultIcon);
  popupLogo.src = pauseStatus
    ? "/icons/gs-cookieblock-96.png"
    : "/icons/cookieblock-96.png";
});
...
document
  .querySelector("#add-exception")
  .addEventListener("click", addGlobalException);
