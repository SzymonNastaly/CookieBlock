...
const restoreSingleCookie = async function (c) {
  ...
  if ((!"expirationDate") in lastUpdate || lastUpdate.session) {
    ...
    console.info(
      `Cookie ${c.name} already expired! Converting to session cookie...`,
    );
    maybeExpiry = null;
  } else {
    ...
    chrome.cookies.set(
      ...
      (result) => {
        if (chrome.runtime.lastError) {
          console.error(
            "Failed to restore cookie: " + chrome.runtime.lastError,
          );
          resolve(false);
        } else {
          resolve(result !== null);
...
const constructCookieListEntry = function (cookies) {
  ...
  for (let c of Object.values(cookies)) {
    ...
    button.addEventListener("click", (ev) => {
      removeOrRestoreCookie(c, button);
    });
...
const constructDomainListEntry = function (domain, path, cookies) {
  ...
  selection.addEventListener("change", (event) => {
    changeExceptionStatus(domain, event.target.value);
  });
  (async () => {
    let domainList = await getStorageValue(
      chrome.storage.sync,
      "cblk_exglobal",
    );
...
const exportCustomLabels = function () {
  ...
  for (let d of Object.keys(cookieHistory)) {
    for (let p of Object.keys(cookieHistory[d])) {
      for (let c of Object.values(cookieHistory[d][p])) {
      ...
      if (allDomains.length > 0) {
        ...
        for (let d of sortedDomains) {
          ...
          for (let path of Object.keys(cookieHistory[d])) {
            constructDomainListEntry(
              sanitizedDomain,
              path,
              cookieHistory[d][path],
            );
          }
        }
      } else {
