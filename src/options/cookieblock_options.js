...
const appendItemToList = async function (addDomain, listID, storageID) {
  ...
  button.addEventListener("click", () => {
    removeItemFromList(addDomain, node, storageID);
  });
...
const setupLocalization = function () {
  ...
  document.getElementById("website_excepts_input").placeholder =
    chrome.i18n.getMessage("exceptionPlaceholderText");
  ...
  document.getElementById("func_excepts_input").placeholder =
    chrome.i18n.getMessage("exceptionPlaceholderText");
  ...
  document.getElementById("analytics_excepts_input").placeholder =
    chrome.i18n.getMessage("exceptionPlaceholderText");
  ...
  document.getElementById("advert_excepts_input").placeholder =
    chrome.i18n.getMessage("exceptionPlaceholderText");
  ...
  document.getElementById("predef-input").placeholder =
    chrome.i18n.getMessage("predefPlaceholder");
...
const setupSettingsPage = async function () {
  ...
  internalCheckbox.checked = await getStorageValue(
    chrome.storage.sync,
    "cblk_useinternal",
  );
...
const updateSelectionsOnStorageChanged = function (changes, area) {
  ...
  if (area === "sync") {
    ...
    if (changedItems.includes("cblk_userpolicy")) {
      let newPolicy = changes["cblk_userpolicy"].newValue;
    ...
    if (changedItems.includes("cblk_exglobal")) {
      emptyAndRestoreList(
        changes["cblk_exglobal"].newValue,
        "cblk_exglobal",
        "website_exceptions",
      );
    }
 ...
 */
const addExcClickListener = function (
  buttonID,
  inputID,
  storageID,
  listID,
...
addExcClickListener(
  "website_excepts_submit",
  "website_excepts_input",
  "cblk_exglobal",
  "website_exceptions",
  "wexcepts-error",
);
...
addExcClickListener(
  "analytics_excepts_submit",
  "analytics_excepts_input",
  "cblk_exanal",
  "analytics_exceptions",
  "analytics-error",
);
addExcClickListener(
  "advert_excepts_submit",
  "advert_excepts_input",
  "cblk_exadvert",
  "advertising_exceptions",
  "advert-error",
);
...
pauseCheckbox.addEventListener("click", (ev) => {
  ...
  chrome.action.setIcon(pauseCheckbox.checked ? grayScaleIcon : defaultIcon);
});
...
const message_openJSON = (inMSG) => {
  chrome.runtime.sendMessage({ open_json: inMSG }, (msg) => {
    if (msg.lastError) {
      ...
      tab.document.getElementById("json_body").textContent = JSON.stringify(
        msg.response,
        null,
        4,
      );
...
cconfigButton.addEventListener("click", (ev) => {
  chrome.tabs.create({
    active: true,
    url: "/options/cookieblock_cconfig.html",
  });
});

// listeners to open JSON documents in new tabs
jsonButton.addEventListener("click", (ev) => {
  message_openJSON("full");
});
necessaryStats.addEventListener("click", (ev) => {
  message_openJSON("necessary");
});
functionalStats.addEventListener("click", (ev) => {
  message_openJSON("functional");
});
analyticsStats.addEventListener("click", (ev) => {
  message_openJSON("analytics");
});
advertisingStats.addEventListener("click", (ev) => {
  message_openJSON("advertising");
});
...
document
  ...
  .addEventListener("click", async (e) => {
    ...
    let pathList = await getStorageValue(
      chrome.storage.sync,
      "cblk_knowncookies",
    );
